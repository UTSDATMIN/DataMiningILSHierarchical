import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
import plotly.express as px
import plotly.graph_objects as go
import numpy as np
from plotly.subplots import make_subplots
from sklearn.preprocessing import StandardScaler
from sklearn.decomposition import PCA  # <-- Tambahkan import ini

# --- Impor modul Anda ---
from methods.dbscan_manual import dbscan_manual, dbscan_summary
from methods.heatmap_cluster import render_heatmap_tab
from methods.clustering_manual import agglomerative_with_history, agglomerative_clustering
from methods.data_loader import load_data
from methods.scoring import encode_answers, build_features
from methods.interpretation import interpret_cluster, describe_cluster
from methods.cluster_profile import render_cluster_profile_tab
from methods.visualization import (
    figures_cluster_distribution,
    figure_learning_style_subplots,
    figure_dendrogram_with_library,
)

st.set_page_config(
    page_title="ILS Clustering Dashboard",
    layout="wide",
    initial_sidebar_state="expanded"
)

# ---------------------- DATA LOAD -----------------------
st.sidebar.header("📂 Data Input")
uploaded_file = st.sidebar.file_uploader(
    "Upload file CSV (optional):",
    type=["csv"],
    help="Upload dataset kustom Anda. Jika tidak, dataset default akan digunakan.",
)

if uploaded_file is not None:
    st.sidebar.success("Custom dataset berhasil digunakan.")
    df, meta_cols, q_cols = load_data(uploaded_file)
else:
    st.sidebar.info("Menggunakan dataset default.")
    df, meta_cols, q_cols = load_data(
        "data/Kuesioner Identifikasi Pola Gaya Belajar Mahasiswa melalui Metode Clustering (Responses) - Form responses 1.csv"
    )

# ---------------------- ENCODE & SCORES -----------------
enc = encode_answers(df, q_cols)
scores, labels, app_cols, kuisoner_data, X = build_features(df, enc)

# ---------------------- SCALER --------------------------
scaler = StandardScaler()
X_scaled = scaler.fit_transform(scores)

# ---- Sidebar clustering method ----
st.sidebar.header("⚙️ Pilih Metode Clustering")
clustering_method = st.sidebar.selectbox("Metode:", ["Hierarchical (Ward)", "DBSCAN (Manual)"])

# ---- Inisialisasi variabel parameter ----
n_clusters = 0
eps = 0.0
min_pts = 0

if clustering_method == "Hierarchical (Ward)":
    n_clusters = st.sidebar.slider("Jumlah Cluster (k):", 2, 10, 3)
else:
    eps = st.sidebar.slider("DBSCAN eps (radius)", 0.1, 2.0, 0.6, 0.1)
    min_pts = st.sidebar.slider("minPts", 2, 10, 3, 1)

# ---- App Layout ----
st.title("Dashboard Analisis Pola Gaya Belajar")
st.subheader("📊 Cluster Visualizations")

# ---- MAIN LOGIC: Conditional Clustering and Tab Rendering ----

# =====================================================================
# =================== BLOK UNTUK HIERARCHICAL CLUSTERING ==============
# =====================================================================
if clustering_method == "Hierarchical (Ward)":
    # 1. Jalankan Clustering
    labels = agglomerative_clustering(X_scaled, n_clusters=n_clusters)
    kuisoner_data["Cluster"] = [f"Cluster {l + 1}" for l in labels]

    # 2. Siapkan data untuk interpretasi
    cluster_means = scores.copy()
    cluster_means["Cluster"] = kuisoner_data["Cluster"]
    cluster_summary = cluster_means.groupby("Cluster").mean().reset_index()
    cluster_summary["Karakteristik"] = cluster_summary.apply(interpret_cluster, axis=1)

    def get_cluster_descriptions(summary, total, data_view):
        texts = []
        for _, row in summary.iterrows():
            texts.append(describe_cluster(row, total, data_view))
        return texts
    
    interpretations = get_cluster_descriptions(cluster_summary, len(kuisoner_data), kuisoner_data)

    # 3. Render Tab Khusus Hierarchical
    tab_names = ["Distribusi Cluster", "Gaya Belajar", "Dendrogram", "Analisis Profil Cluster", "Interpretasi Otomatis", "Cluster Heatmap"]
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(tab_names)

    with tab1:
        st.subheader("📊 Cluster Distribution Overview")
        fig_pie, fig_bar = figures_cluster_distribution(kuisoner_data)
        col1, col2 = st.columns(2)
        with col1:
            st.plotly_chart(fig_pie, use_container_width=True)
        with col2:
            st.plotly_chart(fig_bar, use_container_width=True)
        st.markdown("---")
        st.subheader("📋 Data with Cluster Assignments")
        st.dataframe(kuisoner_data, use_container_width=True, hide_index=True)

    with tab2:
        # (Konten Tab Gaya Belajar - tidak berubah)
        st.subheader("🎨 Learning Style Preferences")
        col1, col2 = st.columns([1, 3])
        with col1:
            st.markdown("### 🎛️ Select Dimension")
            style_choice = st.selectbox("Learning Style Dimension:", ["All (2×2 Subplot)", "Active vs Reflective (AR)", "Sensing vs Intuitive (SI)", "Visual vs Verbal (VV)", "Sequential vs Global (QG)"], label_visibility="collapsed")
        with col2:
            if style_choice == "All (2×2 Subplot)":
                fig_styles = figure_learning_style_subplots(kuisoner_data)
                st.plotly_chart(fig_styles, use_container_width=True)
            else:
                mapping = {"Active vs Reflective (AR)": "AR", "Sensing vs Intuitive (SI)": "SI", "Visual vs Verbal (VV)": "VV", "Sequential vs Global (QG)": "QG"}
                dim = mapping[style_choice]
                counts = kuisoner_data[dim].value_counts()
                fig_single = go.Figure(go.Bar(x=counts.index, y=counts.values, marker_color=['#FF6B6B', '#4ECDC4']))
                fig_single.update_layout(title=f"Distribution of {style_choice}", xaxis_title="Style", yaxis_title="Number of Students")
                st.plotly_chart(fig_single, use_container_width=True)

    with tab3:
        st.subheader("🌳 Dendrogram")
        st.write("Dendrogram menunjukkan bagaimana setiap data point digabungkan menjadi cluster secara hierarkis.")
        fig = figure_dendrogram_with_library(X, kuisoner_data['ID'].tolist())
        st.pyplot(fig)

    with tab4:
        st.subheader("🧠 Analisis Profil Cluster")
        scores_with_cluster = scores.copy()
        scores_with_cluster["Cluster"] = kuisoner_data["Cluster"].values
        render_cluster_profile_tab(scores_with_cluster)

    with tab5:
        st.subheader(f"📝 Interpretasi Otomatis ({n_clusters} Cluster)")
        for t in interpretations:
            st.write(t)
        st.dataframe(cluster_summary.set_index("Cluster"))

    with tab6:
        render_heatmap_tab(X_scaled, scores, agglomerative_with_history, n_clusters)


# =====================================================================
# =================== BLOK UNTUK DBSCAN CLUSTERING ====================
# =====================================================================
else:  # --- DBSCAN (Manual) ---
    # 1. Jalankan Clustering
    labels = dbscan_manual(X_scaled, eps=eps, min_pts=min_pts)
    kuisoner_data["Cluster"] = [f"Cluster {l + 1}" if l != -1 else "Noise" for l in labels]

    # Tampilkan ringkasan di sidebar
    summary = dbscan_summary(labels)
    st.sidebar.write("**📊 DBSCAN Summary (Manual)**")
    st.sidebar.write(f"- Clusters found: {summary['clusters']}")
    st.sidebar.write(f"- Noise points: {summary['noise_points']}")
    
    # 2. Siapkan data untuk interpretasi (kecualikan noise)
    cluster_means = scores.copy()
    cluster_means["Cluster"] = kuisoner_data["Cluster"]
    valid_clusters_df = cluster_means[cluster_means['Cluster'] != 'Noise']
    
    interpretations = ["Tidak ada cluster valid yang terbentuk untuk diinterpretasi."]
    cluster_summary = pd.DataFrame()

    if not valid_clusters_df.empty:
        cluster_summary = valid_clusters_df.groupby("Cluster").mean().reset_index()
        cluster_summary["Karakteristik"] = cluster_summary.apply(interpret_cluster, axis=1)
        
        def get_cluster_descriptions(summary, total, data_view):
            texts = []
            for _, row in summary.iterrows():
                texts.append(describe_cluster(row, total, data_view))
            return texts

        interpretations = get_cluster_descriptions(cluster_summary, len(kuisoner_data), kuisoner_data)

    # 3. Render Tab Khusus DBSCAN
    tab_names = ["Distribusi Cluster & Noise", "Gaya Belajar", "Peta Cluster (Scatter Plot)", "Analisis Profil Cluster", "Interpretasi Otomatis", "Cluster Heatmap"]
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(tab_names)

    with tab1:
        st.subheader("📊 Cluster Distribution & Noise Overview")
        fig_pie, fig_bar = figures_cluster_distribution(kuisoner_data)
        col1, col2 = st.columns(2)
        with col1:
            st.plotly_chart(fig_pie, use_container_width=True)
        with col2:
            st.plotly_chart(fig_bar, use_container_width=True)
        st.markdown("---")
        st.subheader("📋 Data with Cluster Assignments")
        st.dataframe(kuisoner_data, use_container_width=True, hide_index=True)

    with tab2:
        # (Konten Tab Gaya Belajar - tidak berubah)
        st.subheader("🎨 Learning Style Preferences")
        col1, col2 = st.columns([1, 3])
        with col1:
            st.markdown("### 🎛️ Select Dimension")
            style_choice = st.selectbox("Learning Style Dimension:", ["All (2×2 Subplot)", "Active vs Reflective (AR)", "Sensing vs Intuitive (SI)", "Visual vs Verbal (VV)", "Sequential vs Global (QG)"], label_visibility="collapsed", key="dbscan_style_choice")
        with col2:
            if style_choice == "All (2×2 Subplot)":
                fig_styles = figure_learning_style_subplots(kuisoner_data)
                st.plotly_chart(fig_styles, use_container_width=True)
            else:
                mapping = {"Active vs Reflective (AR)": "AR", "Sensing vs Intuitive (SI)": "SI", "Visual vs Verbal (VV)": "VV", "Sequential vs Global (QG)": "QG"}
                dim = mapping[style_choice]
                counts = kuisoner_data[dim].value_counts()
                fig_single = go.Figure(go.Bar(x=counts.index, y=counts.values, marker_color=['#FF6B6B', '#4ECDC4']))
                fig_single.update_layout(title=f"Distribution of {style_choice}", xaxis_title="Style", yaxis_title="Number of Students")
                st.plotly_chart(fig_single, use_container_width=True)

    with tab3:
        st.subheader("✨ Peta Cluster (Scatter Plot via PCA)")
        st.write("Data 4 dimensi direduksi menjadi 2 dimensi menggunakan PCA untuk memvisualisasikan sebaran cluster dan noise.")
        
        pca = PCA(n_components=2)
        X_pca = pca.fit_transform(X_scaled)
        pca_df = pd.DataFrame(data=X_pca, columns=['PC1', 'PC2'])
        pca_df['Cluster'] = kuisoner_data['Cluster']
        
        fig_pca = px.scatter(
            pca_df,
            x='PC1',
            y='PC2',
            color='Cluster',
            title='Peta Cluster 2D (Hasil PCA)',
            color_discrete_map={'Noise': 'grey'},
            hover_data={'Cluster': True}
        )
        st.plotly_chart(fig_pca, use_container_width=True)

    with tab4:
        st.subheader("🧠 Analisis Profil Cluster")
        scores_with_cluster = scores.copy()
        scores_with_cluster["Cluster"] = kuisoner_data["Cluster"].values
        # Filter out noise for profiling
        scores_for_profile = scores_with_cluster[scores_with_cluster['Cluster'] != 'Noise']
        if not scores_for_profile.empty:
            render_cluster_profile_tab(scores_for_profile)
        else:
            st.warning("Tidak ada cluster yang terbentuk (hanya noise), analisis profil tidak dapat ditampilkan.")
    
    with tab5:
        st.subheader("📝 Interpretasi Otomatis")
        if not cluster_summary.empty:
            for t in interpretations:
                st.write(t)
            st.dataframe(cluster_summary.set_index("Cluster"))
        else:
            st.warning("Tidak ada cluster valid untuk diinterpretasi.")

    with tab6:
        dbscan_n_clusters = summary['clusters'] 
        render_heatmap_tab(X_scaled, scores, agglomerative_with_history, n_clusters)


# ---- User Input & Prediction (di luar blok if/else) ----
st.subheader("🤔 Prediksi Gaya Belajar Anda")
questions = {
    "AR": ("Saat belajar, saya lebih suka:", ["Diskusi kelompok", "Belajar sendiri"]),
    "SI": ("Saya lebih suka mempelajari:", ["Fakta nyata", "Konsep abstrak"]),
    "VV": ("Saya lebih mudah memahami materi melalui:", ["Diagram/gambar", "Teks/lisan"]),
    "QG": ("Saya lebih suka belajar dengan:", ["Langkah runtut", "Gambaran besar dulu"]),
}

answers = {}
for key, (qtext, opts) in questions.items():
    answers[key] = st.radio(qtext, opts, key=f"pred_{key}")

if st.button("Submit"):
    user_vector = [
        1 if answers["AR"] == "Diskusi kelompok" else 0,
        1 if answers["SI"] == "Fakta nyata" else 0,
        1 if answers["VV"] == "Diagram/gambar" else 0,
        1 if answers["QG"] == "Langkah runtut" else 0
    ]
    
    dists = np.linalg.norm(X_scaled - scaler.transform([user_vector]), axis=1)
    nearest_idx = np.argmin(dists)
    cluster = kuisoner_data.iloc[nearest_idx]['Cluster']

    st.success("Hasil Profil Anda:")
    st.write(f"**Cluster yang paling sesuai:** {cluster}")
    with st.expander("Lihat detail jawaban Anda"):
        st.write(f"Active–Reflective: {answers['AR']}")
        st.write(f"Sensing–Intuitive: {answers['SI']}")
        st.write(f"Visual–Verbal: {answers['VV']}")
        st.write(f"Sequential–Global: {answers['QG']}")